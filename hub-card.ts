...
function hubDetailsMatch(result: unknown): boolean {
  ...
  return Object.keys(HUB_DETAIL_DISCRIMINATOR).some((key) =>
    hasOwn(details, key),
  );
}
...
function failureOf(fields: Record<string, unknown> | undefined): boolean {
  ...
  for (const key of ["status", "state", "outcome"] as const) {
    if (/(?:fail|error|abort)/.test(textOf(fields[key], 64).toLowerCase()))
      return true;
  }
  return false;
}
...
function stateOf(fields: Record<string, unknown>, fallback: string): string {
  ...
  if (typeof fields["live"] === "boolean")
    return fields["live"] ? "running" : "idle";
  return fallback;
}
...
function subjectOf(fields: Record<string, unknown>, fallback: string): string {
  return (
    recordText(
      fields,
      ["displayName", "name", "id", "jobId", "agent", "to", "from"],
      96,
    ) || fallback
  );
}
...
function pushRecordRows(
  ...
  for (
    let index = 0;
    index < candidates.length && index < inspectLimit;
    index += 1
  ) {
...
function pushTerminalRows(rows: HubRow[], value: unknown, max: number): void {
  ...
  for (
    let index = 0;
    index < value.length && index < inspectLimit;
    index += 1
  ) {
    const raw = value[index];
    const text =
      textOf(raw, 240) ||
      recordText(recordOf(raw), ["text", "line", "message"], 240);
    if (!text) continue;
...
function pushCancelledRows(rows: HubRow[], value: unknown, max: number): void {
  ...
  for (
    ...
    if (fields) {
      rows.push(
        projectedRow("cancelled", fields, String(index + 1), "cancelled"),
      );
      continue;
    }
...
function hubRows(
  ...
  const rows: HubRow[] = [];
  pushRecordRows(rows, details["peers"], "peer", max, (fields) =>
    recordText(fields, ["role", "status"], 160),
  );
  pushRecordRows(rows, details["jobs"], "job", max, (fields) =>
    recordText(
      fields,
      ["label", "task", "resultText", "errorText", "message"],
      180,
    ),
  );
  ...
  pushRecordRows(rows, details["daemon"], "daemon", max, (fields) =>
    recordText(fields, ["state", "pid"], 120),
  );
...
function hubRowCount(details: Record<string, unknown> | undefined): number {
  ...
  if (Array.isArray(details["terminalRows"]))
    total += details["terminalRows"].length;
  if (Array.isArray(details["cancelled"])) total += details["cancelled"].length;
  return total;
}

function hubRowsHaveFailure(
  details: Record<string, unknown> | undefined,
): boolean {
...
function targetOf(
  ...
  if (Array.isArray(ids) && ids.length > 0)
    return `${ids.length} ${ids.length === 1 ? "item" : "items"}`;
  return "session";
}
...
function summaryOf(
  ...
  const state = (
    textOf(details["state"], 32) || textOf(daemon?.["state"], 32)
  ).toLowerCase();
  if (["ready", "running", "stopped", "exited"].includes(state)) return state;
  if (details["timedOut"] === true || details["waited"] === null)
    return "timed out";
  ...
  if (knownCount(details["cancelled"]) > 0)
    return `${knownCount(details["cancelled"])} cancelled`;
  return "completed";
}
...
function hubData(
  ...
  const failure =
    failureOf(recordOf(result)) ||
    failureOf(details) ||
    hubRowsHaveFailure(details);
  return {
    op,
...
export function renderHubCardLines(
  ...
  if (hidden > 0)
    lines.push(cardDetailLine(theme, width, `… ${hidden} more items`));
  return lines;
}
...
export function renderHubCard(
  ...
  card.addChild({
    render: (width: number): readonly string[] =>
      renderHubCardLines(theme, width, args, result, options, fingerprint) ??
      [],
  });
  markFlush?.(card);
  return card;
