...
The plugin registers five choices in `/settings` → **Composer Shape**:
...
- **Minimal Output · Below Dock** — the Top Dock's complete status line on a row of its own below the frame: project and Git at the left, provider usage and context gauge in the middle, and the mode chip pinned to the right edge. The frame itself carries no status.

The docks drop the host's `session_name` segment — the auto-generated task title — from the status row: a task name is not session state, and presets that list it would otherwise paint a whole title (often one that reads like the failure it describes) where the model, project, and gauge belong. The docks render a single status row: multi-line status text is flattened onto it, and status text that is a JavaScript crash dump (a failed status provider, or an extension that pushes an exception message as its hook status) is dropped instead of being painted where the project, model, and gauge belong. The original Bottom Dock and Top Dock styles preserve OMP's live `statusLine.*` theme colors. The two Grayscale styles strip inherited ANSI colors from the prompt chrome and repaint the full frame with explicit equal-RGB grays: darker rules, medium status text, and a brighter neutral prompt gutter. Editor text remains unchanged. Both docks prefix the working head ahead of project/Git while a run is active: the configured indicator frame plus elapsed run time (`12s`/`2m`/`3h`), with no prefix when idle. The native model name stays exactly where OMP emits it; only a duplicated second copy is removed, so the model appears once. Provider usage formats concisely to the primary window (showing `5h` if present, otherwise `7d`, never both), displaying remaining quota left ($100\% - \text{used}\%$) while retaining the reset countdown timer. While idle at the prompt, composer chrome auto-refreshes periodically (`composerRefreshInterval`, default `60` seconds, range `1..3600`) to keep reset countdowns and provider quota current without requiring user interaction.
...
The sticky Todo widget hydrates as one collapsed summary row on session start only when the current session already has a nonempty todo list. It renders between the live thinking block and the working status row (any other `aboveEditor` widget keeps its own slot). Expanding it shows the full retained list; `standardMaxRows` does not apply to Todo. Empty new sessions keep the widget absent until a todo tool result creates work. Rows carry the native checkbox glyphs from the host theme (`checkbox.checked`/`unchecked`), with an ASCII fallback: empty box for pending with the label color unchanged, checked box for settled, and in-between boxes for other statuses (`[!]` blocked, `[-]` dropped; `☒`/`⊟` when the theme uses glyph boxes). The in-progress row keeps its `◐`/spinner indicator. When an item settles the row holds for one tick and a line sweeps across its label, which then stays struck through.

Twenty-three shadows delegate to native execution with native schemas — `bash`, `read`, `grep`, `glob`, `write`, `edit`, `eval`, `web_search`, and the fifteen tools with no dedicated layout — as does the whole `mcp__*` family behind one opt-out. Task and Hub are not shadows: `cards/native-tool-card-skin.ts` observes public `ToolExecutionComponent` updates, renders only verified Task/Hub state, and fails open to the native renderer. Write shows a bounded, path-aware syntax-highlighted input preview while running and settled; `web_search` shows source count/provider and expands to bold titles with dim URLs; expanded `edit` cards use project-relative file branches, per-file stats, continuation rails, explicit diff markers, and hunk separators; `eval` replaces the boxed output panel.
...
Live reasoning streams in the animated widget above the composer (placed above the AI working status message); `registerAssistantThinkingRenderer` is supplemental-only. `hideThinkingBlock: true` (the shipped `minimal-output.yml`) hides the native thinking block and the transcript thought surfaces — the transcript row and the settled block. The plugin's own widget is the replacement for those, so it keeps streaming the live reasoning either way (and keeps its 4-row slot when idle so the composer does not jump). With `hideThinkingBlock: false`, the transcript also shows a `Thought` row with its duration followed by the bounded latest lines. Thought and thinking rows are composed through `cards/card-primitives.ts` (`thoughtRailLine`), so the rail sits on the card content column and the text uses the card `dim` token at the configured `opacity`. Spill files land in `$TMPDIR/omp-minimal-*.log`.
...
- `surfaces/` — `composer-shapes.ts` (five prompt docks: frame rules, grayscale palette, shape registry), `composer-editor.ts` (editor re-framing, below-dock status row, `composerRefreshInterval` polling), `composer-status.ts` (status text pipeline and live providers), `composer-gauge.ts` (context gauge), `composer-primitives.ts` (host glyph vocabulary and ANSI/width helpers), `thinking-widget.ts`, `todo-widget.ts`/`todos-header.ts`/`todo-hud.ts`, `commands.ts` (slash commands + shortcut), warning/assistant-commentary/read-group skins, `scrolling-text.ts`.
